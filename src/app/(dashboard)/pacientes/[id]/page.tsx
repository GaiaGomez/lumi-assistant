...
export default async function PatientProfilePage({ params, searchParams }: Props) {
  ...
  const activeTab = tabQuery === 'clinical-profile' ? 'clinical-profile' : 'overview'
  const basePatientHref = `/pacientes/${id}`

  const settingsPromise = fetchSettings(supabase, user.id)
  const clinicalProfilePromise = getPatientClinicalProfile(id)

  const [{ data: patient }, { data: appointments }, notes, clinicalProfile] = await Promise.all([
  ...
  return (
    <div className="relative pb-1">
      ...
      <PatientHeaderCard
        ...
        nameHref={`${basePatientHref}?tab=clinical-profile`}
      />

      <div className="mb-2.5 flex flex-wrap gap-2">
        <Link
          href={basePatientHref}
          className={`${activeTab === 'overview' ? 'btn-action' : 'btn-subtle'} px-3 py-1.5 text-[12px]`}
        >
          Resumen
        </Link>
        <Link
          href={`${basePatientHref}?tab=clinical-profile`}
          className={`${activeTab === 'clinical-profile' ? 'btn-action' : 'btn-subtle'} px-3 py-1.5 text-[12px]`}
        >
          Ficha clínica
        </Link>
      </div>
